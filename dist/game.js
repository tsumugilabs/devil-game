...
function update(dt){if(state==='dead'){deadTime+=dt;for(const p of particles){p.x+=p.vx*dt;p.y+=p.vy*dt;p.vy+=850*dt;p.t+=dt;}if(deadTime>.45)$('overlay').classList.remove('hidden');return;}if(state!=='playing')return;const l=levels[level];elapsed+=dt;x+=l.speed*dt;const previousBottom=y+size;vy+=(held&&vy<0?1550:2050)*dt;y+=vy*dt;
 for(const h of l.holes)if(x>h[2])shown.add(h[0]);const hole=l.holes.some(h=>shown.has(h[0])&&x+size*.65>h[0]&&x+size*.35<h[1]);
 if(y+size>=floor&&!hole&&previousBottom<=floor+8&&vy>=0){y=floor-size;vy=0;grounded=true;}else if(hole)grounded=false;
...
function draw(){const l=levels[level],cam=Math.max(0,x-210);ctx.fillStyle='#eae8df';ctx.fillRect(0,0,1000,480);ctx.strokeStyle='#dad8cf';ctx.lineWidth=1;for(let i=0;i<22;i++){const gx=i*60-(cam*.2)%60;ctx.beginPath();ctx.moveTo(gx,0);ctx.lineTo(gx,480);ctx.stroke();}for(let i=0;i<9;i++){ctx.beginPath();ctx.moveTo(0,i*60);ctx.lineTo(1000,i*60);ctx.stroke();}
 text(String(level+1).padStart(2,'0'),830,125,100,'#d9d7cf');ctx.save();ctx.translate(-cam,0);ctx.fillStyle='#292c2a';ctx.fillRect(cam,floor,1000,110);ctx.fillStyle='#77786f';ctx.fillRect(cam,floor,1000,3);
